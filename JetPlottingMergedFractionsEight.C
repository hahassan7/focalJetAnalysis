...
void JetPlottingMergedFractionsEight() // loops over R and saves to same file
{
    int pTHardRange[] = {5, 10, 20, 30, 40, 60, 100, 200, 1000};

    bool normalizeXsec = false;
    ...
    const int nEBins = 6;                                                                      // 16;
    const int npTBins = 8;                                                                     // 16;
    ...
    for (int iNorm = 0; iNorm < nNorm; iNorm++)
    {
        ...
        TFile *fout;
        fout = new TFile(Form("Data20230816/Fractions/En20230816_pythia8_JetJet_%d-%dGeV_Merged_TotalOutput_0Mass_NoNorm.root", pTHardRange[iNorm], pTHardRange[iNorm + 1]), "RECREATE");
        ...
        for (int Rvalue = 0; Rvalue < nR; Rvalue++)
        {
            ...
            hPartNeutral[Rvalue] = new TH1F(Form("hPartNeutral_R%d", Rvalue), Form("All particle level jet neutral energy fraction distribution, R = %0.1f", Rvals[Rvalue]), 21, -0.025, 1.025);
            ; // neutralfrac all not just matched
            ...
            if (normalizeXsec)
            {
                hDetECAL[Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                hPartNeutralMatch[Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                hDetECALAll[Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                hPartNeutral[Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
            }
            ...
            for (int ieta = 0; ieta < nEtaBins - 1; ++ieta)
            {
                ...
                hPartNeutralEta[Rvalue][ieta] = new TH1F(Form("hPartNeutralEta_%d_R%d", ieta, Rvalue), Form("All particle level jet neutral energy fraction distribution, %s, R = %0.1f", etaRange[ieta].Data(), Rvals[Rvalue]), 21, -0.025, 1.025);
                // neutralfrac all not just matched
                ...
                if (hDetECALEta[Rvalue][ieta]->GetEntries() == 0)
                    hDetECALEta[Rvalue][ieta]->Fill(-2);
                if (hPartNeutralMatchEta[Rvalue][ieta]->GetEntries() == 0)
                    hPartNeutralMatchEta[Rvalue][ieta]->Fill(-2);
                if (hDetECALAllEta[Rvalue][ieta]->GetEntries() == 0)
                    hDetECALAllEta[Rvalue][ieta]->Fill(-2);
                if (hPartNeutralEta[Rvalue][ieta]->GetEntries() == 0)
                    hPartNeutralEta[Rvalue][ieta]->Fill(-2);

                if (normalizeXsec)
                {
                    hDetECALEta[Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hPartNeutralMatchEta[Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hDetECALAllEta[Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hPartNeutralEta[Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                }
                ...
                for (int iconst = 0; iconst < nConst; ++iconst)
                {
                    ...
                    hPartNeutralEtaConst[Rvalue][ieta][iconst] = new TH1F(Form("hPartNeutralEta_%d_Const_%d_R%d", ieta, iconst, Rvalue), Form("All particle level jet neutral energy fraction distribution, %s, R = %0.1f", etaRange[ieta].Data(), Rvals[Rvalue]), 21, -0.025, 1.025);
                    ; // neutralfrac all not just matched
                    ...
                    if (hDetECALEtaConst[Rvalue][ieta][iconst]->GetEntries() == 0)
                        hDetECALEtaConst[Rvalue][ieta][iconst]->Fill(-2);
                    if (hPartNeutralMatchEtaConst[Rvalue][ieta][iconst]->GetEntries() == 0)
                        hPartNeutralMatchEtaConst[Rvalue][ieta][iconst]->Fill(-2);
                    if (hDetECALAllEtaConst[Rvalue][ieta][iconst]->GetEntries() == 0)
                        hDetECALAllEtaConst[Rvalue][ieta][iconst]->Fill(-2);
                    if (hPartNeutralEtaConst[Rvalue][ieta][iconst]->GetEntries() == 0)
                        hPartNeutralEtaConst[Rvalue][ieta][iconst]->Fill(-2);

                    if (normalizeXsec)
                    {
                        hDetECALEtaConst[Rvalue][ieta][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hPartNeutralMatchEtaConst[Rvalue][ieta][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hDetECALAllEtaConst[Rvalue][ieta][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hPartNeutralEtaConst[Rvalue][ieta][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    }
                }
            }
            ...
            for (int iconst = 0; iconst < nConst; ++iconst)
            {
                ...
                if (hDetECALConst[Rvalue][iconst]->GetEntries() == 0)
                    hDetECALConst[Rvalue][iconst]->Fill(-2);
                if (hPartNeutralMatchConst[Rvalue][iconst]->GetEntries() == 0)
                    hPartNeutralMatchConst[Rvalue][iconst]->Fill(-2);
                if (hDetECALAllConst[Rvalue][iconst]->GetEntries() == 0)
                    hDetECALAllConst[Rvalue][iconst]->Fill(-2);
                if (hPartNeutralConst[Rvalue][iconst]->GetEntries() == 0)
                    hPartNeutralConst[Rvalue][iconst]->Fill(-2);

                if (normalizeXsec)
                {
                    hDetECALConst[Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hPartNeutralMatchConst[Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hDetECALAllConst[Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hPartNeutralConst[Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                }
            }
            ...
            for (int iE = 0; iE < nEBins - 1; ++iE)
            {
                ...
                if (hDetECALE[iE][Rvalue]->GetEntries() == 0)
                    hDetECALE[iE][Rvalue]->Fill(-2);
                if (hPartNeutralMatchE[iE][Rvalue]->GetEntries() == 0)
                    hPartNeutralMatchE[iE][Rvalue]->Fill(-2);
                if (hDetECALAllE[iE][Rvalue]->GetEntries() == 0)
                    hDetECALAllE[iE][Rvalue]->Fill(-2);
                if (hPartNeutralE[iE][Rvalue]->GetEntries() == 0)
                    hPartNeutralE[iE][Rvalue]->Fill(-2);

                if (normalizeXsec)
                {
                    hDetECALE[iE][Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hPartNeutralMatchE[iE][Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hDetECALAllE[iE][Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hPartNeutralE[iE][Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                }
                ...
                for (int ieta = 0; ieta < nEtaBins - 1; ++ieta)
                {
                    ...
                    if (hDetECALEEta[iE][Rvalue][ieta]->GetEntries() == 0)
                        hDetECALEEta[iE][Rvalue][ieta]->Fill(-2);
                    if (hPartNeutralMatchEEta[iE][Rvalue][ieta]->GetEntries() == 0)
                        hPartNeutralMatchEEta[iE][Rvalue][ieta]->Fill(-2);
                    if (hDetECALAllEEta[iE][Rvalue][ieta]->GetEntries() == 0)
                        hDetECALAllEEta[iE][Rvalue][ieta]->Fill(-2);
                    if (hPartNeutralEEta[iE][Rvalue][ieta]->GetEntries() == 0)
                        hPartNeutralEEta[iE][Rvalue][ieta]->Fill(-2);

                    if (normalizeXsec)
                    {
                        hDetECALEEta[iE][Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hPartNeutralMatchEEta[iE][Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hDetECALAllEEta[iE][Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hPartNeutralEEta[iE][Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    }
                }
                ...
                for (int iconst = 0; iconst < nConst; ++iconst)
                {
                    ...
                    if (hDetECALEConst[iE][Rvalue][iconst]->GetEntries() == 0)
                        hDetECALEConst[iE][Rvalue][iconst]->Fill(-2);
                    if (hPartNeutralMatchEConst[iE][Rvalue][iconst]->GetEntries() == 0)
                        hPartNeutralMatchEConst[iE][Rvalue][iconst]->Fill(-2);
                    if (hDetECALAllEConst[iE][Rvalue][iconst]->GetEntries() == 0)
                        hDetECALAllEConst[iE][Rvalue][iconst]->Fill(-2);
                    if (hPartNeutralEConst[iE][Rvalue][iconst]->GetEntries() == 0)
                        hPartNeutralEConst[iE][Rvalue][iconst]->Fill(-2);

                    if (normalizeXsec)
                    {
                        hDetECALEConst[iE][Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hPartNeutralMatchEConst[iE][Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hDetECALAllEConst[iE][Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hPartNeutralEConst[iE][Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    }
                }
            }
            ...
            for (int ipT = 0; ipT < npTBins - 1; ++ipT)
            {
                ...
                if (hDetECALpT[ipT][Rvalue]->GetEntries() == 0)
                    hDetECALpT[ipT][Rvalue]->Fill(-2);
                if (hPartNeutralMatchpT[ipT][Rvalue]->GetEntries() == 0)
                    hPartNeutralMatchpT[ipT][Rvalue]->Fill(-2);
                if (hDetECALAllpT[ipT][Rvalue]->GetEntries() == 0)
                    hDetECALAllpT[ipT][Rvalue]->Fill(-2);
                if (hPartNeutralpT[ipT][Rvalue]->GetEntries() == 0)
                    hPartNeutralpT[ipT][Rvalue]->Fill(-2);

                if (normalizeXsec)
                {
                    hDetECALpT[ipT][Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hPartNeutralMatchpT[ipT][Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hDetECALAllpT[ipT][Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    hPartNeutralpT[ipT][Rvalue]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                }
                ...
                for (int ieta = 0; ieta < nEtaBins - 1; ++ieta)
                {
                    ...
                    if (hDetECALpTEta[ipT][Rvalue][ieta]->GetEntries() == 0)
                        hDetECALpTEta[ipT][Rvalue][ieta]->Fill(-2);
                    if (hPartNeutralMatchpTEta[ipT][Rvalue][ieta]->GetEntries() == 0)
                        hPartNeutralMatchpTEta[ipT][Rvalue][ieta]->Fill(-2);
                    if (hDetECALAllpTEta[ipT][Rvalue][ieta]->GetEntries() == 0)
                        hDetECALAllpTEta[ipT][Rvalue][ieta]->Fill(-2);
                    if (hPartNeutralpTEta[ipT][Rvalue][ieta]->GetEntries() == 0)
                        hPartNeutralpTEta[ipT][Rvalue][ieta]->Fill(-2);

                    if (normalizeXsec)
                    {
                        hDetECALpTEta[ipT][Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hPartNeutralMatchpTEta[ipT][Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hDetECALAllpTEta[ipT][Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hPartNeutralpTEta[ipT][Rvalue][ieta]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    }
                }
                ...
                for (int iconst = 0; iconst < nConst; ++iconst)
                {
                    ...
                    if (hDetECALpTConst[ipT][Rvalue][iconst]->GetEntries() == 0)
                        hDetECALpTConst[ipT][Rvalue][iconst]->Fill(-2);
                    if (hPartNeutralMatchpTConst[ipT][Rvalue][iconst]->GetEntries() == 0)
                        hPartNeutralMatchpTConst[ipT][Rvalue][iconst]->Fill(-2);
                    if (hDetECALAllpTConst[ipT][Rvalue][iconst]->GetEntries() == 0)
                        hDetECALAllpTConst[ipT][Rvalue][iconst]->Fill(-2);
                    if (hPartNeutralpTConst[ipT][Rvalue][iconst]->GetEntries() == 0)
                        hPartNeutralpTConst[ipT][Rvalue][iconst]->Fill(-2);

                    if (normalizeXsec)
                    {
                        hDetECALpTConst[ipT][Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hPartNeutralMatchpTConst[ipT][Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hDetECALAllpTConst[ipT][Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                        hPartNeutralpTConst[ipT][Rvalue][iconst]->Scale(normalizations[iNorm] / nFolders[iNorm]);
                    }
                }
            }
        }
    ...
    gSystem->Exec("hadd Data20230816/Fractions/EnMerged_FractionsEight_0Mass_NoNorm.root Data20230816/Fractions/En20230816_pythia8_JetJet_*_0Mass_NoNorm.root");
}