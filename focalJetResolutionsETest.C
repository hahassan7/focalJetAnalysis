...
void focalJetResolutionsETest( 
    ...
    Int_t r = 1; Int_t e=0;
    TLegend* leg2 = GetAndSetLegend2(0.15, 0.79, 0.33, 0.95, textSizeLabels[1], 1, "", 42, 0.35);      //(0.63, 0.53, 0.84, 0.75, textSizeLabels[1], 1, "", 42, 0.35);     
    ...
    for (Int_t p = 0; p <3; p++){
      ...
      histDeltaPt_bins[r][e][exampleBins[p]]->SetYTitle("1/#it{k} d#it{N}/d#Delta E");
      histDeltaPt_bins[r][e][exampleBins[p]]->SetXTitle("#Delta E = (E_{part}-E_{part})/E_{part} ");
    ...
    drawLatexAdd("#it{R} = 0.6",0.7,0.945-4*1.1*textSizeLabels[0], textSizeLabels[0],kFALSE, kFALSE, kFALSE); 
    ...
    cSlicesJES->Update();
    cSlicesJES->SaveAs(Form("figs/FitEJetEscaleProj_%s.pdf",radiusOut[r].Data()));
