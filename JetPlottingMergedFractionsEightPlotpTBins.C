...
void JetPlottingMergedFractionsEightPlotpTBins(int NormValue=8) {
    ...
    TH1F* hPartNeutralMatchpT[npTBins - 1][nR];

	TH1F* hDetECALpTEta[npTBins - 1][nR][nEtaBins - 1];
	TH1F* hPartNeutralMatchpTEta[npTBins - 1][nR][nEtaBins - 1];

	TH1F *hPartNeutralMatchpTConst[npTBins-1][nR][nConst]; 
    ...
    for (int Rvalue = 0; Rvalue < nR; Rvalue++) {
        for (int eb = 0; eb < npTBins - 1; eb++) {
            ...
            hPartNeutralMatchpT[eb][Rvalue] = (TH1F*)fin->Get(Form("hPartNeutralMatchpT_%d_R%d", eb, Rvalue));NormalizeToProb(hPartNeutralMatchpT[eb][Rvalue]);
            ...
            for (int ieta = 0; ieta < nEtaBins - 1; ++ieta) {
		        ...
		        hPartNeutralMatchpTEta[eb][Rvalue][ieta] = (TH1F*)fin->Get(Form("hPartNeutralMatchpT_%dEta_%d_R%d", eb, ieta, Rvalue));NormalizeToProb(hPartNeutralMatchpTEta[eb][Rvalue][ieta]);
		        hPartNeutralMatchpTEta[eb][Rvalue][ieta]->GetYaxis()->SetRangeUser(limitYEtamin,limitYEta);
            }

            for (int iconst = 0; iconst < nConst; ++iconst) {
		        hPartNeutralMatchpTConst[eb][Rvalue][iconst] = (TH1F*)fin->Get(Form("hPartNeutralMatchpT_%dConst_%d_R%d", eb, iconst, Rvalue));NormalizeToProb(hPartNeutralMatchpTConst[eb][Rvalue][iconst]);
		        hPartNeutralMatchpTConst[eb][Rvalue][iconst]->GetYaxis()->SetRangeUser(limitYconstmin,limitYconst);
            }
        }
    }
    PlotHistogramsMultiPad(hDetECALpTEta, "ECAL Fraction vs. Eta for Different R Values", "E_{jet}/E_{jet}^{det}", "fractionFigs/ptmin10/fractionspT/EtaDetECALMultiPanel");
    PlotHistogramsMultiPad(hPartNeutralMatchpTEta, "Neutral Fraction vs. Eta for Different R Values", "E_{neutral}/E_{jet}^{part}", "fractionFigs/ptmin10/fractionspT/EtaPartNeutralMatchMultiPanel");

    PlotHistogramsMultiPadConst(hPartNeutralMatchpTConst, "Neutral Fraction vs. Constituent Bins for Different R Values", "E_{neutral}/E_{jet}^{part}", "fractionFigs/ptmin10/fractionspT/ConstPartNeutralMatchMultiPanel");

    // Plot histograms using multi-panel
    PlotHistogramsMultiPanel(hDetECALpT, "Matched detector level jet ECAL energy fraction distribution", "E_{ECAL}/E_{jet}^{det}", "fractionFigs/ptmin10/fractionspT/DetECALMultiPanel.png");
    PlotHistogramsMultiPanel(hPartNeutralMatchpT, "Matched particle level jet neutral energy fraction distribution", "E_{neutral}/E_{jet}^{part}", "fractionFigs/ptmin10/fractionspT/PartNeutralMatchMultiPanel.png");
...
void PlotHistogramsMultiPanel(TH1F* histos[npTBins - 1][nR], const TString& title, const TString& xTitle, const TString& outputFileName) {
    ...
    for (int eb = 0; eb < npTBins - 1; eb++) {
        ...
        TLatex* titleLatex = new TLatex(0.15, 0.92, Form("pT_{jet} = %0.0f - %0.0f", JetpTBorders[eb], JetpTBorders[eb + 1]));
        titleLatex->SetNDC();
...
void PlotHistogramsMultiPad(TH1F* histos[npTBins-1][nR][nEtaBins-1], TString canvasTitle, TString xAxisTitle, TString outputFileName) {
    ...
    for (int Rvalue = 0; Rvalue < nR; Rvalue++) {
        ...
        c->Divide(3, 3); // Three columns and three rows for npTBins-1
...
void PlotHistogramsMultiPadConst(TH1F* histos[npTBins - 1][nR][nConst], TString title, TString xTitle, TString outputFileName) {
    ...
    for (int Rvalue = 0; Rvalue < nR; Rvalue++) {
        ...
        cMultiPad->Divide(3, 3); // Divide canvas into 3x3 pads (for 9 pt bins)
