/*  Macro for plotting deltapT and deltaE
    distributions. Set R index as argument
*/

double GetMedian( TH1D * h);

void NewPlotter(int Rvalue = 2) 
{
    
    TFile *jetFile = TFile::Open("/home/lmh/alice/fromPuhti/v19/EtaTest/Eta_20230127_PythiaMBTrig-2/Merged.root");
    ...
    const Float_t EtaBinBorders[nEtaBins] = {3.7, 4.5, 5.0};
    ...
    TFile *fout = new TFile(Form("Eta_Check_OutputR%d.root", int(Rvals[Rvalue] * 10)), "RECREATE");
    //histograms
    ...
    TH2D *hRespMatrix_E = new TH2D("hRespMatrix_E", Form("Detector response matrix, R=%0.1f;#it{E}_{jet}^{det} (GeV);#it{E}_{jet}^{part} (GeV)", Rvals[Rvalue]), 3000, 0, 2000, 3000, 0, 2000);
