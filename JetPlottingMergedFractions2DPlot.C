...
void JetPlottingMergedFractions2DPlot(int NormValue=8)
{
...
//setting NormValue = 8 if merged file in use
    ...
    if(NormValue==8)fin = new TFile("Data20230728/2D/Merged.root", "READ");
    ...
    for (Int_t e = 0; e < 3; e++){
      for (Int_t r = 2; r < nR; r++){
        cCorr->Clear();
         SetStyleHistoTH2ForGraphs(hRespMatrixpT[r][exampleBins[e]], "#it{E}_{ECAL}/#it{E}_{jet}^{det} (GeV)", "#it{E}_{neutral}/#it{E}_{jet}^{part} (GeV)", 0.85*textSize,textSize, 0.85*textSize,textSize, 0.95, 0.9, 510, 510, 42, 62);
        ...
        drawLatexAdd("ALICE simulation, pp #sqrt{#it{s}} = 14 TeV",0.14,0.99-1*0.9*textSize, 0.85*textSize,kFALSE, kFALSE, kFALSE);  
        drawLatexAdd("FoCal upgrade",0.14,0.99-2*0.9*textSize, 0.85*textSize,kFALSE, kFALSE, kFALSE);  
        drawLatexAdd("jets, anti-#it{k}_{T}, R=0.6",0.85,0.99-1*0.9*textSize, 0.85*textSize,kFALSE, kFALSE, kTRUE);  
        drawLatexAdd(Form("%.0f< #it{p}_{T,det}<%.0f GeV/#it{c}", JetpTBorders[exampleBins[e]], JetpTBorders[exampleBins[e]+1]),0.85,0.99-2*0.9*textSize, 0.85*textSize,kFALSE, kFALSE, kTRUE);  
        cCorr->Update();
...
void ChangeTitleOfCanvas(TCanvas* cCanvas, TString sTitle){
    ...
    tex->Draw();
    if(SettingLogZ==1) cCanvas->SetLogz();
}