...
void SpectraBoth()
{
    ...
    TFile *fileDet = TFile::Open("Data20230816/Spectra/ScaledDetMerged_0Mass.root", "READ");
    TFile *file = TFile::Open("Data20230816/Spectra/ScaledTruthMerged.root", "READ");
...
void NormalizeToProb(TH1F *hHisto)
{
    ...
    if (hHisto != NULL)
        hHisto->Scale(factor / scalingFactor, "width"); // hHisto->SetLineWidth(1);
    if (hHisto != NULL)
        hHisto->Rebin(2);
}
