...
void JetPlottingMergedFileEnergyJES(int Rvalue = 0)
{   
    const Int_t nR = 3; //5
    const Float_t Rvals[nR] = {0.2, 0.4, 0.6}; // Cone radii

    TFile *jetFile = TFile::Open(Form("Data20230728/JES/EnMergedR%d.root", int(Rvals[Rvalue] * 10)));
    ...
    TFile *fout = new TFile(Form("Data20230728/JES/EneMerged_OutputR%d.root", int(Rvals[Rvalue] * 10)), "RECREATE");
    ...
    for(int nef=0; nef < NEFbins; nef++){
        hMedianENEF[nef]->GetXaxis()->SetTitle("E^{part} (GeV)");
        hMedianENEF[nef]->GetYaxis()->SetTitle("Mean or median");

        hMeanENEF[nef]->GetXaxis()->SetTitle("E^{part} (GeV)");
        hMeanENEF[nef]->GetYaxis()->SetTitle("Mean or median");

        hSDENEF[nef]->GetXaxis()->SetTitle("E^{part} (GeV)");
        hSDENEF[nef]->GetYaxis()->SetTitle("Standard deviation");
    }
