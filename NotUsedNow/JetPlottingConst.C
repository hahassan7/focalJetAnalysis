...
void JetPlottingConst()
{   
    //TFile *jetFile = TFile::Open("/home/lmh/alice/fromPuhti/v19/20230127_PythiaMBTrig-2_const/Merged.root");

    //TFile *jetFile = TFile::Open("/home/lmh/alice/fromPuhti/v110/20230319_pythia-pi0-trig_ptmin-2_const/Merged.root");
    TFile *jetFile = TFile::Open("/home/lmh/alice/fromPuhti/v110/20230328_pythia-pi0-trig_ptmin-2_AlCuts/Merged.root");
    ...
    int constMin = 2; //min number of constituents in matchd jet
    ...
    TFile *fout = new TFile(Form("20230328_pythia-pi0-trig_ptmin-2_AlCuts_OutputR%d.root", int(Rvals[Rvalue] * 10)), "RECREATE");
    //histograms
    ...
    TH2D *hRespMatrix_pT = new TH2D("hRespMatrix_pT", Form("Detector response matrix, R=%0.1f;#it{p}_{T}^{det} (GeV/c);#it{p}_{T}^{part} (GeV/c)", Rvals[Rvalue]), 200, 0, 50, 200, 0, 50);
    ...
    for (int iR = 0; iR < nR; iR++)
    {
        ...
        jetTree->Draw(Form("jetpT>>hjetpT_%d", iR), Form("jetR==%d && jetEta>%f && jetEta<%f && jetParts_match > %d", int(Rvals[iR] * 10), etaMin, etaMax, constMin), "goff");

        jetTree->Draw(Form("jetE>>hjetE_%d", iR), Form("jetR==%d && jetEta>%f && jetEta<%f && jetParts_match > %d", int(Rvals[iR] * 10), etaMin, etaMax, constMin), "goff");

        TruthjetTree->Draw(Form("truthjetpT>>hTruthjetpT_%d", iR), Form("truthjetR==%d && truthjetEta>%f && truthjetEta<%f && jetParts_truth > %d", int(Rvals[iR] * 10), etaMin, etaMax, constMin), "goff");

        TruthjetTree->Draw(Form("truthjetE>>hTruthjetE_%d", iR), Form("truthjetR==%d && truthjetEta>%f && truthjetEta<%f && jetParts_truth > %d", int(Rvals[iR] * 10), etaMin, etaMax, constMin), "goff");
    }

    jetTree->Draw("jetE_match:jetE>>hRespMatrix_E", Form("jetR==%d && jetEta>%f && jetEta<%f && jet_distmatch<%f && jetParts_match > %d", int(Rvals[Rvalue] * 10), etaMin, etaMax, Rvals[Rvalue]*0.6, constMin), "goff");
    jetTree->Draw("jetpT_match:jetpT>>hRespMatrix_pT", Form("jetR==%d && jetEta>%f && jetEta<%f && jet_distmatch<%f && jetParts_match > %d", int(Rvals[Rvalue] * 10), etaMin, etaMax, Rvals[Rvalue]*0.6, constMin), "goff");

    for (int iE = 0; iE < nEtaBins-1; ++iE) //eta loop
    {
        hRespMatrix_pT_Eta[iE] = new TH2D(Form("hRespMatrix_pT_Eta_%d", iE), Form("Detector response matrix, eta %0.1f - %0.1f, R=%0.1f;#it{p}_{T}^{det} (GeV/c);#it{p}_{T}^{part} (GeV/c)", EtaBinBorders[iE], EtaBinBorders[iE+1], Rvals[Rvalue]), 200, 0, 50, 200, 0, 50);
   ...
   for (int ipt = 0; ipt < nPtBins-1; ++ipt) 
    {
        ...
        jetTree->Draw(Form("(jetE-jetE_match)/jetE_match>>hjetRatioE_%d", ipt), Form("jetR==%d && jetpT_match>=%d  && jetpT_match<%d  && jetEta>3.4+%f && jetEta<5.5-%f && jetEta_match>3.4+%f && jetEta_match<5.5-%f && jet_distmatch<%f && jetParts_match > %d", int(Rvals[Rvalue] * 10), int(JetPtBorders[ipt]), int(JetPtBorders[ipt+1]),Rvals[Rvalue],Rvals[Rvalue],Rvals[Rvalue],Rvals[Rvalue], Rvals[Rvalue]*0.6, constMin), "goff"); 
