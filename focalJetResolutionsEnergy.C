    ...
    TString inputfileR04 = "Data20230816/JES/EneMerged_OutputR6_pT_0Mass.root")
{
  ...
  for (int i = 0; i < 3; i++)
  {
    if (i == 0)
    {
      histResponseMat_E[0][i] = (TH2D *)fileR02->Get("hRespMatrix_E");
      histMean_E[0][i] = (TH1D *)fileR02->Get("hMeanE");
      histMedian_E[0][i] = (TH1D *)fileR02->Get("hMedianE");
      histSigma_E[0][i] = (TH1D *)fileR02->Get("hSDE");
      hgausMeansE = (TH1D *)fileR02->Get("hgausMeansE");
      hgausSDE = (TH1D *)fileR02->Get("hgausSDE");
      for (int p = 0; p < maxNEbins; p++)
      {
        histDeltaE_bins[0][i][p] = (TH1D *)fileR02->Get(Form("hjetRatioE_%d", p));
    ...
    {
      histResponseMat_E[0][i] = (TH2D *)fileR02->Get(Form("hRespMatrix_E_Eta_%i", i - 1));
      histMean_E[0][i] = (TH1D *)fileR02->Get(Form("hEtaMeanE_%i", i - 1));
      histMedian_E[0][i] = (TH1D *)fileR02->Get(Form("hEtaMedianE_%i", i - 1));
      histSigma_E[0][i] = (TH1D *)fileR02->Get(Form("hEtaSDE_%i", i - 1));
      for (int p = 0; p < maxNEbins; p++)
      {
        histDeltaE_bins[0][i][p] = (TH1D *)fileR02->Get(Form("hjetRatioE_Eta_%i_%i", i - 1, p));
        histDeltaE_bins[0][i][p]->Scale(histDeltaE_bins[0][i][p]->GetBinWidth(1));
        histDeltaE_bins[0][i][p]->Scale(1. / histDeltaE_bins[0][i][p]->Integral());
      }
    }
    std::cout << "R = 0.2 \t" << histMean_E[0][i] << "\t" << histSigma_E[0][i] << "\t" << histResponseMat_E[0][i] << std::endl;
    histResponseMat_E[0][i]->Rebin2D(9, 15);
    for (int enty = 1; enty <= histResponseMat_E[0][i]->GetNbinsY(); enty++)
    {
      double sumX = 0.0;
      for (int entx = 1; entx <= histResponseMat_E[0][i]->GetNbinsX(); entx++)
      {
        // sum all x
        sumX += histResponseMat_E[0][i]->GetBinContent(histResponseMat_E[0][i]->GetBin(entx, enty));
      }
      for (int entx = 1; entx <= histResponseMat_E[0][i]->GetNbinsX(); entx++)
      {
        // normalize with sum
        double oldBin = histResponseMat_E[0][i]->GetBinContent(histResponseMat_E[0][i]->GetBin(entx, enty));
        double newBin = 0.0;
        if (sumX != 0.0)
        {
          newBin = oldBin / sumX;
          histResponseMat_E[0][i]->SetBinContent(entx, enty, newBin);
        }
      }
      std::cout << "sum = " << sumX << std::endl;
    }
  }
  std::cout << "nbins y  = " << histResponseMat_E[0][0]->GetNbinsY() << std::endl;
  std::cout << "nbins x  =" << histResponseMat_E[0][0]->GetNbinsX() << std::endl;
  std::cout << " y goes from" << histResponseMat_E[0][0]->GetYaxis()->GetBinCenter(1) << " to " << histResponseMat_E[0][0]->GetYaxis()->GetBinCenter(histResponseMat_E[0][0]->GetNbinsY()) << std::endl;
  std::cout << "x  goes from" << histResponseMat_E[0][0]->GetXaxis()->GetBinCenter(1) << " to " << histResponseMat_E[0][0]->GetXaxis()->GetBinCenter(histResponseMat_E[0][0]->GetNbinsX()) << std::endl;
  ...
  for (int i = 0; i < 3; i++)
  {
    if (i == 0)
    {
      histResponseMat_E[1][i] = (TH2D *)fileR04->Get("hRespMatrix_E");
      histMean_E[1][i] = (TH1D *)fileR04->Get("hMeanE");
      histMedian_E[1][i] = (TH1D *)fileR04->Get("hMedianE");
      histSigma_E[1][i] = (TH1D *)fileR04->Get("hSDE");
      for (int p = 0; p < maxNEbins; p++)
      {
        histDeltaE_bins[1][i][p] = (TH1D *)fileR04->Get(Form("hjetRatioE_%d", p));
    ...
    {
      histResponseMat_E[1][i] = (TH2D *)fileR04->Get(Form("hRespMatrix_E_Eta_%i", i - 1));
      histMean_E[1][i] = (TH1D *)fileR04->Get(Form("hEtaMeanE_%i", i - 1));
      histMedian_E[1][i] = (TH1D *)fileR04->Get(Form("hEtaMedianE_%i", i - 1));
      histSigma_E[1][i] = (TH1D *)fileR04->Get(Form("hEtaSDE_%i", i - 1));
      for (int p = 0; p < maxNEbins; p++)
      {
        histDeltaE_bins[1][i][p] = (TH1D *)fileR04->Get(Form("hjetRatioE_Eta_%i_%i", i - 1, p));
        histDeltaE_bins[1][i][p]->Scale(histDeltaE_bins[1][i][p]->GetBinWidth(1));
        histDeltaE_bins[1][i][p]->Scale(1. / histDeltaE_bins[1][i][p]->Integral());
      }
    }
  ...
  for (int r = 0; r < RMax; r++)
  {
    ...
    cJES->cd();
    cJES->Update();
    cJES->SaveAs(Form("figs/Energy/JES_%s.pdf", radiusOut[r].Data()));
    ...
    cJER->cd();
    cJER->Update();
    cJER->SaveAs(Form("figs/Energy/JER_%s.pdf", radiusOut[r].Data()));
  }
  ...
  for (int e = 0; e < 3; e++)
  {
    ...
    for (int r = 0; r < RMax; r++)
    {
      ...
      else
      {
        ...
        DrawSetMarker(histMedian_E[r][e], marker[e], markerS[e] * 1.2, colors[e], colors[e]);
        DrawSetMarker(hgausMeansE, marker[e + 1], markerS[e] * 1.2, colors[e + 1], colors[e + 1]);
      }
      histMean_E[r][e]->Draw("same");
      histMedian_E[r][e]->Draw("same");
      hgausMeansE->Draw("same");
      leg2->AddEntry(histMean_E[r][e], "mean", "p"); // leg2->AddEntry(histMean_E[r][e], radiusLabel[r].Data(), "p");
      leg2->AddEntry(histMedian_E[r][e], "median", "p");
      leg2->AddEntry(hgausMeansE, "mean from Gaussian fit", "p");
    }
    leg2->Draw("same");
    drawLatexAdd("ALICE simulation, pp #sqrt{#it{s}} = 14 TeV", 0.95, 0.965 - 1 * 1.1 * textSize, textSize, kFALSE, kFALSE, kTRUE);
    drawLatexAdd("FoCal upgrade", 0.95, 0.965 - 2 * 1.1 * textSize, textSize, kFALSE, kFALSE, kTRUE);
    ...
    cJES->cd();
    cJES->Update();
    auto fline = new TF1("fline", "0.0", 200, 3200);
    fline->SetLineColor(kGray);
    fline->SetLineStyle(2);
    fline->Draw("same");
    cJES->SaveAs(Form("figs/Energy/gausJES_%s.pdf", etaOut[e].Data()));
    ...
    for (int r = 0; r < RMax; r++)
    {
      ...
      else
      {
        ...
        DrawSetMarker(hgausSDE, marker[e + 1], markerS[e] * 1.2, colors[e + 1], colors[e + 1]);
      }
      histSigma_E[r][e]->Draw("same");
      hgausSDE->Draw("same");
      // leg3->AddEntry((TObject*)0, radiusLabel[r].Data(), "");
      leg3->AddEntry(histSigma_E[r][e], "standard deviation", "p");
      leg3->AddEntry(hgausSDE, "standard deviation from Gaussian fit", "p");
    }
    leg3->Draw("same");
    drawLatexAdd("ALICE simulation, pp #sqrt{#it{s}} = 14 TeV", 0.95, 0.965 - 1 * 1.1 * textSize, textSize, kFALSE, kFALSE, kTRUE);
    drawLatexAdd("FoCal upgrade", 0.95, 0.965 - 2 * 1.1 * textSize, textSize, kFALSE, kFALSE, kTRUE);
    ...
    cJER->cd();
    cJER->Update();
    cJER->SaveAs(Form("figs/Energy/gausJER_%s.pdf", etaOut[e].Data()));
  }

  // Style projection
  TCanvas *cCorr = new TCanvas("cCorr", "cCorr", 2 * 900, 2 * 860);
  // DrawPaperCanvasSettings(cCorr, 0.095, 0.11, 0.02, 0.1);1386
  DrawPaperCanvasSettings(cCorr, 0.14, 0.121, 0.092, 0.11);
  ...
  for (int e = 0; e < 3; e++)
  {
    ...
    {
      cCorr->Clear();
      SetStyleHistoTH2ForGraphs(histResponseMat_E[r][e], "#it{E}_{det} (GeV)", "#it{E}_{part} (GeV)", 0.75 * textSize * 1.1, textSize * 1.1, 0.75 * textSize * 1.1, textSize * 1.1, 0.85, 1.28, 510, 510, 42, 62);
      histResponseMat_E[r][e]->GetZaxis()->SetLabelSize(0.85 * textSize);
      histResponseMat_E[r][e]->GetZaxis()->SetLabelOffset(- 0.005);
      // histResponseMat_E[r][e]->Rebin2D(6,6);
      // histResponseMat_E[r][e]->GetZaxis()->SetRangeUser(0.000001, 10);
      histResponseMat_E[r][e]->GetXaxis()->SetRangeUser(JESMinX, JESMaxX);
      histResponseMat_E[r][e]->GetYaxis()->SetRangeUser(JESMinX, JESMaxX);
      ...
      cCorr->Update();
      cCorr->SaveAs(Form("figs/Energy/ResponseMatrix_%s_%s.pdf", etaOut[e].Data(), radiusOut[r].Data()));
    }
  }
  ...
  for (int r = 0; r < RMax; r++)
  {
    ...
    cSlicesJES->cd();
    cSlicesJES->Update();
    cSlicesJES->SaveAs(Form("figs/Energy/JetEscaleProj_%s.pdf", radiusOut[r].Data()));
  }
  ...
  for (int e = 0; e < 3; e++)
  {
    ...
    cSlicesJES->cd();
    cSlicesJES->Update();
    cSlicesJES->SaveAs(Form("figs/Energy/JetEscaleProj_%s.pdf", etaOut[e].Data()));
  }
}
