...
{   
    ...
    //TFile *jetFile = TFile::Open("/home/lmh/alice/fromPuhti/20230124_Pythia8JetJet/Merged.root");
    TFile *jetFile = TFile::Open("/home/lmh/alice/fromPuhti/v19/20230127_PythiaMBTrig-2_NewCode/Merged.root");
    ...
    const double JetEBorders[nPtBins] = {20.0, 40.0, 60.0, 80.0, 100.0, 150.0, 200.0, 400.0, 600.0, 1000.0};
    ...
    TFile *fout = new TFile(Form("NewCodev1-9_E_OutputR%d.root", int(Rvals[Rvalue] * 10)), "RECREATE");
    //histograms
    ...
    hMedianE = new TH1D("hMedianE", "Mean and median of #DeltaE distribution",nPtBins-1, JetEBorders);
    hMeanpT = new TH1D("hMeanpT", "Mean and median of #Deltap_{T} distribution",nPtBins-1, JetPtBorders); 
    hMeanE = new TH1D("hMeanE", "Mean and median of #DeltaE distribution",nPtBins-1, JetEBorders); 
    hSDpT = new TH1D("hSDpT", "Standard deviation of #Deltap_{T} distribution",nPtBins-1, JetPtBorders);
    hSDE = new TH1D("hSDE", "Standard deviation of #DeltaE distribution",nPtBins-1, JetEBorders);
    ...
    for (int iE = 0; iE < nEtaBins-1; ++iE) //eta loop
    {
        ...
        hEtaMedianE[iE] = new TH1D(Form("hEtaMedianE_%d", iE), Form("Mean and median of #DeltaE distribution, eta %0.1f - %0.1f, R=%0.1f", EtaBinBorders[iE], EtaBinBorders[iE+1], Rvals[Rvalue]),nPtBins-1, JetEBorders);
        ...
        hEtaMeanE[iE] = new TH1D(Form("hEtaMeanE_%d", iE), Form("Mean and median of #DeltaE distribution, eta %0.1f - %0.1f, R=%0.1f", EtaBinBorders[iE], EtaBinBorders[iE+1], Rvals[Rvalue]),nPtBins-1, JetEBorders); 
        ...
        hEtaSDE[iE] = new TH1D(Form("hEtaSDE_%d", iE), Form("Standard deviation of #DeltaE distribution, eta %0.1f - %0.1f, R=%0.1f", EtaBinBorders[iE], EtaBinBorders[iE+1], Rvals[Rvalue]),nPtBins-1, JetEBorders);  
    }
   ...
   for (int ipt = 0; ipt < nPtBins-1; ++ipt) 
    {
        ...
        for (int iE = 0; iE < nEtaBins-1; ++iE)
        {
            ...
            hjetRatioE_Eta[iE][ipt] = new TH1D(Form("hjetRatioE_Eta_%d_%d", iE, ipt), Form("Jet-by-jet #Deltap_{T} distribution, #eta: %0.1f - %0.1f (det jet), p_{T}: %d - %d GeV/c", EtaBinBorders[iE], EtaBinBorders[iE+1], int(JetEBorders[ipt]), int(JetEBorders[ipt+1])), 50, -1.0, 1.0);
